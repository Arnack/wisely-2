...
async function generateExperts() {
  ...
  const expertProfiles = expertUsers.map((user, index) => ({
    ...
    subscription_type: Math.random() > 0.3 ? 'premium' : 'free',
...
async function generateAvailabilitySlots(expertProfiles) {
...
async function main() {
  try {
    ...
    console.log('')
    console.log('🎉 You can now browse experts and book consultations!')
